...
class TVRecObjectiveFn(ObjectiveFn):
    ...
    def __call__(self, x: np.ndarray) -> float:
        ...
        return 0.5 * np.linalg.norm(self.forward_map @ u - self.true_img) ** 2
    ...
    def hessian(self, x: np.ndarray) -> float:
        ...
        """
        A = self.forward_map.T @ self.forward_map
        d = np.concatenate(
            (
                A.tocoo(),
...
class RecStateConstraintFn(ConstraintFn):
    def __init__(
        self,
        forward_map: sp.csr_matrix,
        noisy_img: np.ndarray,
        gradient_op: sp.csr_matrix,
        parameter_size: int = 1,
    ):
        self.forward_map = forward_map
...
class PrimalConstraintFn(ConstraintFn):
    def __init__(self, gradient_op: sp.csr_matrix, parameter_size: int = 1):
        self.gradient_op = gradient_op
        self.M, self.N = gradient_op.shape
        self.R = self.M // 2
        self.parameter_size = parameter_size
...
class TVReconstruction:
    def __init__(
        ...
        self.true_img = true_img.flatten()
        self.noisy_img = dmg_img.flatten()
        self.gamma = gamma
        self.epsilon = epsilon
        ...
        self.nlp = OptimizationProblem(
            TVRecObjectiveFn(forward_map, self.true_img, self.M, self.N, epsilon),
            eq_constraint_funcs=[
                RecStateConstraintFn(forward_map, self.noisy_img, self.K),
                PrimalConstraintFn(self.K),
                DualConstraintFn(self.K, self.Kx, self.Ky),
            ],
            ineq_constraint_funcs=[
                BoundConstraintFn(self.M, self.N),
                ComplementarityConstraintFn(self.M, self.N, t=t),
            ],
        )
